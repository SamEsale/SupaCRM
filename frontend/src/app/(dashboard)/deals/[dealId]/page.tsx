        ...
        async function loadDealDetail(): Promise<void> {
            try {
                ...
                try {
                    ...
                    resolvedQuotes = linkedQuotes.items ?? [];
                    ...
                    const invoiceResults = await Promise.all(
                        resolvedQuotes.map(async (quote) => {
                            ...
                            return invoiceResponse.items ?? [];
                        }),
                    );

                    const invoicesById = new Map<string, Invoice>();
                    invoiceResults.flat().forEach((invoice) => {
                        invoicesById.set(invoice.id, invoice);
                    });
                    resolvedInvoices = Array.from(invoicesById.values());
                } catch (error) {
                    console.warn("Linked quote or invoice lookup failed on deal detail:", error);
                    if (isMounted) {
                        setActionError("Linked quote or invoice lookup failed. You can still work from the deal.");
                    }
                }
                ...
                <>
                    ...
                    <section className="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
                        ...
                        {relatedInvoices.length === 0 ? (
                            <p className="mt-4 text-sm text-slate-600">
                                No invoices have been linked from this deal&apos;s quotes yet.
                            </p>
                        ) : (
                            <div className="mt-4 space-y-3">
