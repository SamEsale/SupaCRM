...
export default function DealsPage() {
    ...
    const loadPageData = useCallback(async (): Promise<void> => {
        try {
            ...
            console.error("Failed to load deals page data:", error);
            setErrorMessage("The deals page could not be loaded from the backend.");
        } finally {
            setIsLoading(false);
        }
    }, []);
    ...
    const filteredDeals = useMemo(() => {
        return deals.filter((deal) => {
            const matchesSearch = matchesDeal(
                deal,
                searchTerm,
                companies,
                contacts,
            ...
            const matchesCompany =
                selectedCompanyId === "" || deal.company_id === selectedCompanyId;

            return matchesSearch && matchesCompany;
        });
    }, [deals, searchTerm, selectedCompanyId, companies, contacts, products]);
        ...
        <main className="space-y-6">
            <section className="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2">
                    <input
                        ...
                        placeholder="Search deals by name, company, contact, product, stage, status, or amount"
            ...
            {isLoading ? (
                <section className="rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
                    ...
                    <p className="mt-2 text-sm text-slate-600">
                        Fetching deals, companies, contacts, and products from the backend API.
                    </p>
                </section>
            ) : errorMessage ? (
