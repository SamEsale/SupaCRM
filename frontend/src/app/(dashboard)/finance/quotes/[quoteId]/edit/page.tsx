...
export default function EditQuotePage() {
        ...
        async function loadFormData(): Promise<void> {
            try {
                ...
                try {
                    ...
                    console.warn("Deals could not be loaded for quote edit enrichment:", error);
                }
                ...
                setQuoteNumber(quoteResponse.number);
                ...
                setForm({
                    ...
                    contact_id: quoteResponse.contact_id ?? "",
                    deal_id: quoteResponse.deal_id ?? "",
                    product_id: quoteResponse.product_id ?? "",
                    issue_date: quoteResponse.issue_date,
                    expiry_date: quoteResponse.expiry_date,
                    currency: quoteResponse.currency,
                    total_amount: quoteResponse.total_amount,
                    notes: quoteResponse.notes ?? "",
                });
            } catch (error) {
                console.error("Failed to load quote edit data:", error);
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>): Promise<void> {
        ...
        if (!quoteId || submitLockRef.current || isSubmitting) {
            return;
        }
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900">
                            {quoteNumber ? `Edit ${quoteNumber}` : "Edit Quote"}
                        </h1>
                        <p className="mt-2 text-sm text-slate-600">
                            Update quote details. Quote status is managed separately.
                        </p>
                    </div>

                    <Link
                        href={quoteId ? `/finance/quotes/${quoteId}` : "/finance/quotes"}
                ...
                <section className="max-w-2xl rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
                    <form onSubmit={handleSubmit} noValidate className="space-y-4">
                        ...
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            >
                                {isSubmitting ? "Saving quote..." : "Save Changes"}
                            </button>
