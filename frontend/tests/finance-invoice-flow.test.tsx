...
describe("finance invoice flow", () => {
    ...
    it("falls back to manual invoice mode when quote prefill fails and does not submit source_quote_id", async () => {
        mocks.getQuoteById.mockRejectedValue(new Error("Quote not found"));
        ...
        await waitFor(() => {
            expect(
                screen.getByText(
                    /Quote prefill could not be loaded\. Continue creating the invoice manually\./i,
                ),
            ).toBeTruthy();
        });
        ...
        expect(mocks.createInvoice.mock.calls[0]?.[0]).not.toHaveProperty("source_quote_id");
    });
